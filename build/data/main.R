...
oicdata1 <- read_excel("/home/alec/Projects/Brookings/older-industrial-cities/build/data/OIC Appendix Table.xlsx", sheet="APPENDIX_TABLE", na=c("","NA","N/A"))
...
oicdata <- left_join(oicdata1, nsfnih) %>% inner_join(bagap) %>% filter(is.na(oic) | oic==1) %>%
...
count_missing <- function(v){return(sum(is.na(v)))}
num_missing <- oicdata %>% summarise_all(count_missing)
sum(is.na(oicdata)) #epop_ch (and sig) [1], nsfnih [12], hub_share [5], score [3]
